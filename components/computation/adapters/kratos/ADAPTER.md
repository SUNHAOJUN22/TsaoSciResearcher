...
- Required Python module(s): `KratosMultiphysics`
...
- Candidate backends: `cpu`, `openmp`, `mpi`, `cuda`, `task-parallel`
- Preferred planning order: `mpi`, `cuda`, `openmp`, `task-parallel`, `cpu`
- Parallel strategies: `mpi`, `openmp`, `task-parallel`, `solver-native`
- Execution mode: `hybrid-library`
- Edge suitability: `limited`
- Candidate libraries: `cusparse`, `cublas`
...
- GPU support is application and build dependent.
- Model part, solver, linear algebra, precision, and convergence must be qualified.

Candidate Kratos acceleration metadata only; no installed application or GPU result is claimed.

Run `python -m tsao_computation probe-accelerators` and `python -m tsao_computation plan-acceleration kratos` before preparing an accelerated run. A GPU, compiler, Python package, or CUDA-X library detected on the host does not prove that the selected executable was built for that backend. Compare CPU and accelerated results for completion, convergence, observables, precision, determinism, conservation, uncertainty, applicability, wall time, memory, energy, and thermal behavior. Preserve a bounded CPU fallback.
...
Use the closest workflow example under `examples/`, then replace every system-specific value with contract-backed data. Examples are templates, not evidence that Kratos Multiphysics is installed.

## Scripts

- `python -m tsao_computation probe`
- `python -m tsao_computation probe-accelerators`
- `python -m tsao_computation plan-acceleration kratos`
