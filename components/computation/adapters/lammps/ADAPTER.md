...
- Candidate executable(s): `lmp`, `lmp_mpi`
...
- Preferred planning order: `cuda`, `hip`, `sycl`, `mpi`, `openmp`, `cpu`
- Parallel strategies: `mpi`, `openmp`, `multi-gpu`, `solver-native`
- Execution mode: `solver-native`
- Edge suitability: `limited`
- Candidate libraries: `kokkos`, `cufft`, `nccl`
...
- GPU, KOKKOS, OPENMP, INTEL, and other packages are build-dependent.
- Pair styles, precision, neighbor settings, MPI ranks, threads, and GPU mapping must be benchmarked.

Candidate LAMMPS acceleration metadata only; package availability and scientific speedup require live evidence.

Run `python -m tsao_computation probe-accelerators` and `python -m tsao_computation plan-acceleration lammps` before preparing an accelerated run. A GPU, compiler, Python package, or CUDA-X library detected on the host does not prove that the selected executable was built for that backend. Compare CPU and accelerated results for completion, convergence, observables, precision, determinism, conservation, uncertainty, applicability, wall time, memory, energy, and thermal behavior. Preserve a bounded CPU fallback.
...
Use the closest workflow example under `examples/`, then replace every system-specific value with contract-backed data. Examples are templates, not evidence that LAMMPS is installed.

## Scripts

- `python -m tsao_computation probe`
- `python -m tsao_computation probe-accelerators`
- `python -m tsao_computation plan-acceleration lammps`
