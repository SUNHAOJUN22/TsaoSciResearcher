...
- Required Python module(s): `openmm`
...
- Candidate backends: `cpu`, `cuda`, `hip`, `opencl`, `task-parallel`
- Preferred planning order: `cuda`, `hip`, `opencl`, `task-parallel`, `cpu`
- Parallel strategies: `solver-native`, `multi-gpu`, `task-parallel`
- Execution mode: `hybrid-library`
- Edge suitability: `suitable`
- Candidate libraries: `cufft`, `tensorrt`
...
- Platform and precision support depend on the installed OpenMM build and hardware.
- Device selection, determinism, precision, force agreement, and energy drift require validation.

Candidate OpenMM platform metadata only; no live dynamics, convergence, or GPU speedup is claimed.

Run `python -m tsao_computation probe-accelerators` and `python -m tsao_computation plan-acceleration openmm` before preparing an accelerated run. A GPU, compiler, Python package, or CUDA-X library detected on the host does not prove that the selected executable was built for that backend. Compare CPU and accelerated results for completion, convergence, observables, precision, determinism, conservation, uncertainty, applicability, wall time, memory, energy, and thermal behavior. Preserve a bounded CPU fallback.
...
Use the closest workflow example under `examples/`, then replace every system-specific value with contract-backed data. Examples are templates, not evidence that OpenMM is installed.

## Scripts

- `python -m tsao_computation probe`
- `python -m tsao_computation probe-accelerators`
- `python -m tsao_computation plan-acceleration openmm`
