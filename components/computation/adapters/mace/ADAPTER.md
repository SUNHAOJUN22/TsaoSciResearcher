...
- Slug: `mace`
- Workflow: `machine-learning-potential`
- Maturity: `A3`
...
- `TSC-051` `training-set-design` — Interatomic-potential training-set design
- `TSC-052` `active-learning-loop` — Active-learning sampling loop
- `TSC-053` `descriptor-selection` — Atomic-descriptor selection
- `TSC-054` `force-energy-validation` — Force and energy validation
- `TSC-055` `extrapolation-detection` — Potential extrapolation detection
- `TSC-056` `deployment-benchmark` — ML-potential deployment benchmark

## Prerequisites

- Candidate executable(s): `python`
- Required Python module(s): `mace`
...
- Candidate backends: `cpu`, `cuda`, `task-parallel`
- Preferred planning order: `cuda`, `task-parallel`, `cpu`
- Parallel strategies: `multi-gpu`, `task-parallel`, `solver-native`
- Execution mode: `hybrid-library`
- Edge suitability: `suitable`
- Candidate libraries: `cuequivariance`, `cutensor`, `cublas`, `nccl`, `tensorrt`

Probe hints:

- python -c "import mace"
- python -c "import cuequivariance"

Limitations:

- cuEquivariance compatibility depends on model architecture, framework, package, CUDA, and device.
- Training and inference must retain reference accuracy, precision, determinism, and applicability evidence.

Candidate MACE/cuEquivariance integration metadata only; no installed kernels, trained model, or speedup is claimed.

Run `python -m tsao_computation probe-accelerators` and `python -m tsao_computation plan-acceleration mace` before preparing an accelerated run. A GPU, compiler, Python package, or CUDA-X library detected on the host does not prove that the selected executable was built for that backend. Compare CPU and accelerated results for completion, convergence, observables, precision, determinism, conservation, uncertainty, applicability, wall time, memory, energy, and thermal behavior. Preserve a bounded CPU fallback.
...
Use the closest workflow example under `examples/`, then replace every system-specific value with contract-backed data. Examples are templates, not evidence that MACE is installed.

## Scripts

- `python -m tsao_computation probe`
- `python -m tsao_computation probe-accelerators`
- `python -m tsao_computation plan-acceleration mace`
