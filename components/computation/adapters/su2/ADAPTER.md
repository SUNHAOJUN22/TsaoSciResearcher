...
- Candidate executable(s): `SU2_CFD`
...
- Interfaces: `cli`
- Candidate backends: `cpu`, `openmp`, `mpi`, `task-parallel`
- Preferred planning order: `mpi`, `openmp`, `task-parallel`, `cpu`
- Parallel strategies: `mpi`, `openmp`, `task-parallel`
...
- Accelerator capabilities are not assumed and depend on the exact build and upstream support.
- Mesh, linear solver, turbulence model, convergence, and scaling require live benchmarks.

Candidate CPU/MPI planning metadata only; no SU2 accelerator build or live solution is claimed.

Run `python -m tsao_computation probe-accelerators` and `python -m tsao_computation plan-acceleration su2` before preparing an accelerated run. A GPU, compiler, Python package, or CUDA-X library detected on the host does not prove that the selected executable was built for that backend. Compare CPU and accelerated results for completion, convergence, observables, precision, determinism, conservation, uncertainty, applicability, wall time, memory, energy, and thermal behavior. Preserve a bounded CPU fallback.
...
Use the closest workflow example under `examples/`, then replace every system-specific value with contract-backed data. Examples are templates, not evidence that SU2 is installed.

## Scripts

- `python -m tsao_computation probe`
- `python -m tsao_computation probe-accelerators`
- `python -m tsao_computation plan-acceleration su2`
