...
- Candidate backends: `cpu`, `mpi`, `cuda`, `task-parallel`
- Preferred planning order: `cuda`, `mpi`, `task-parallel`, `cpu`
- Parallel strategies: `mpi`, `task-parallel`, `solver-native`
- Execution mode: `hybrid-library`
- Edge suitability: `limited`
- Candidate libraries: `cupy`, `cublas`, `cufft`
...
- GPU functionality is limited to supported GPAW modes and versions.
- Mode, precision, domain decomposition, and CPU reference comparison are mandatory.

Candidate GPAW GPU metadata only; installed support and scientific equivalence are not assumed.

Run `python -m tsao_computation probe-accelerators` and `python -m tsao_computation plan-acceleration gpaw` before preparing an accelerated run. A GPU, compiler, Python package, or CUDA-X library detected on the host does not prove that the selected executable was built for that backend. Compare CPU and accelerated results for completion, convergence, observables, precision, determinism, conservation, uncertainty, applicability, wall time, memory, energy, and thermal behavior. Preserve a bounded CPU fallback.
...
Use the closest workflow example under `examples/`, then replace every system-specific value with contract-backed data. Examples are templates, not evidence that GPAW is installed.

## Scripts

- `python -m tsao_computation probe`
- `python -m tsao_computation probe-accelerators`
- `python -m tsao_computation plan-acceleration gpaw`
