...
description: Scale and method selection workflow with explicit contracts, preflight, convergence, physical validation, uncertainty, provenance, and fail-closed acceptance.
---
...
Use this workflow for scale and method selection tasks after the root contract establishes the observable, scale, method, evidence, and resource boundary. Routing keywords include `scale`, `method`, `model`, `fidelity`.
...
- `TSC-001` `problem-decomposition` — Scientific problem decomposition
- `TSC-002` `scale-boundary-identification` — Scale-boundary identification
- `TSC-003` `method-fitness-assessment` — Method fitness assessment
- `TSC-004` `multi-fidelity-plan` — Multi-fidelity computation planning
