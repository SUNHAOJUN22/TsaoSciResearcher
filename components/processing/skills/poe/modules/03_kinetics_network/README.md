# 03_kinetics_network — Kinetic network and moments

## Purpose

Define initiation, propagation, comonomer incorporation, transfer and deactivation with transparent moments.
...
qualified chemistry, concentration/time units, rate constants with provenance. Inputs use SI units unless a field explicitly declares and converts another unit. `contract.schema.json` defines the machine-readable input and output envelope.
...
rates, monomer consumption, moments, Mn/Mw and composition predictions. Every output carries `status`, evidence IDs, applicability, uncertainty, blockers and approval state.
...
Historical POE_Kinetics.m is CONTROLLED_HISTORICAL_EVIDENCE and is never executed as the trusted kernel. Missing evidence returns `NOT_EVALUATED` or `HOLD`; structural inconsistency returns `FAIL`. No missing item is converted into an assumed PASS.
