# 01_product_cqa — Product and CQA

## Purpose

Translate application needs into grade targets, CQAs, measurement boundaries and acceptance criteria.
...
product requirements, application environment, reference grades. Inputs use SI units unless a field explicitly declares and converts another unit. `contract.schema.json` defines the machine-readable input and output envelope.
...
CQA matrix, test methods, uncertainty and acceptance status. Every output carries `status`, evidence IDs, applicability, uncertainty, blockers and approval state.
...
No grade target becomes a production specification without qualified measurements and named approval. Missing evidence returns `NOT_EVALUATED` or `HOLD`; structural inconsistency returns `FAIL`. No missing item is converted into an assumed PASS.

## Tests

Positive, boundary and malicious-input tests are located in `../../tests/test_poe_alpha6_p0.py` and `../../tests/test_poe_alpha6_package.py`. Required Gate: `G2`.
