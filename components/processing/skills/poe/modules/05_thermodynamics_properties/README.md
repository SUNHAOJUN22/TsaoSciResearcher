# 05_thermodynamics_properties — Thermodynamics and properties

## Purpose

Qualify PC-SAFT, SRK, REFPROP, STEAMNBS or custom correlations over a declared envelope.
...
components, ranges, parameter sources, benchmarks and query state. Inputs use SI units unless a field explicitly declares and converts another unit. `contract.schema.json` defines the machine-readable input and output envelope.
...
method qualification status, errors, extrapolation and HOLD reasons. Every output carries `status`, evidence IDs, applicability, uncertainty, blockers and approval state.
...
A simulator method selection alone is not evidence; out-of-domain use returns HOLD. Missing evidence returns `NOT_EVALUATED` or `HOLD`; structural inconsistency returns `FAIL`. No missing item is converted into an assumed PASS.
...
`skills.poe.properties` provides regression error metrics, power-law apparent viscosity and heat-transfer margin calculations. Property-method qualification remains bounded by benchmarks and applicability ranges.
