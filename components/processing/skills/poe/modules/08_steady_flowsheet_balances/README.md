# 08_steady_flowsheet_balances — Steady flowsheet and balances

## Purpose

Build simulator-neutral streams/equipment and independently close mass and energy balances.
...
components, streams, equipment, property qualification and convergence. Inputs use SI units unless a field explicitly declares and converts another unit. `contract.schema.json` defines the machine-readable input and output envelope.
...
closed balances, stream table, topology and HOLD/FAIL status. Every output carries `status`, evidence IDs, applicability, uncertainty, blockers and approval state.
...
Unclosed recycle or nonconverged cases cannot PASS. Missing evidence returns `NOT_EVALUATED` or `HOLD`; structural inconsistency returns `FAIL`. No missing item is converted into an assumed PASS.
