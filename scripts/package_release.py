...
GENERATED_DIR_PREFIXES = ("dist-", "dist_", "build-", "build_", "release-", "release_")
MAX_FILES = 10_000
...
def _is_generated_path(relative: Path) -> bool:
    ...
    return top in EXCLUDED_DIRS or top.endswith(".egg-info") or top.startswith(GENERATED_DIR_PREFIXES)
...
def source_files(root: Path = ROOT) -> list[Path]:
    ...
    for path in root.rglob("*"):
        ...
        if _is_generated_path(relative) or any(
            part in EXCLUDED_DIRS or part.endswith(".egg-info") for part in relative.parts[1:]
        ):
            continue
        if path.is_symlink():
            raise ValueError(f"release source contains symbolic link: {relative.as_posix()}")
        if (
            relative.as_posix() == "SHA256SUMS"
            or path.name in EXCLUDED_ROOT_FILES
