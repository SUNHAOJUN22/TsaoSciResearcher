    ...
    try:
        ...
        for r in read_jsonl(a.evidence_jsonl):
            if r['source']['kind']=='paper':
                ...
                if not (DOI.match(loc) or URL.match(loc) or loc.lower().startswith(('pmid:','arxiv:','isbn:'))): warnings.append(f'{r["evidence_id"]}: paper locator is not a recognized DOI/URL/identifier')
