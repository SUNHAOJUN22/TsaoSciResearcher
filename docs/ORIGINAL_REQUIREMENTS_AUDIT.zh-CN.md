...
本审计以最初的 `TsaoSciResearcher Design(2).docx`、322 项 AI for Science Skill 目录和当前 v0.6.0 源码为对象。判断严格区分：**原生确定性实现、科研控制/合同层、宿主工具执行、外部计算执行和人工审批**。

## 总结

- Excel 中 **322/322** 个 Skill Slug 已进入机器可读能力目录，缺失 **0**。
- 代码另增加 **18** 个运行时/治理能力，总能力合同 **340**。
...
|---|---|---|---|---|
| A01 | 综合科研方法学中枢 | **完整实现** | 已建立单入口 Skill、15 个科研工作流、340 个能力合同和统一状态/验证层。 | `SKILL.md`<br>`workflows/`<br>`capabilities/v2/index.json`<br>`tsao_researcher/` |
...
| A13 | 能力索引 | **完整实现** | Excel 的 322 个 Skill Slug 全部进入 v2 目录，缺失 0；另增加 18 个运行时核心能力，总计 340。 | `capabilities/v2/capabilities.json`<br>`capabilities/v2/index.json`<br>`tests/test_design_compliance.py` |
...
| A18 | 测试、发布与可复现性 | **完整实现** | 最终验证为 212 项测试、项目覆盖率 96.371%、分支覆盖率 93.128%、21/21 Mutation、确定性 ZIP、wheel/sdist 安装验证。 | `docs/QUALITY_BASELINE.json`<br>`scripts/run_tests.py`<br>`scripts/package_release.py`<br>`scripts/validate_distribution.py` |
